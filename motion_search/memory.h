...
struct aligned_deallocator {
  template <typename data_t> void operator()(data_t *p) {
...
template <typename data_t>
inline aligned_unique_ptr<data_t> AlignedAlloc(const size_t numItems) {
  const size_t size = sizeof(data_t) * numItems;
  std::unique_ptr<data_t, aligned_deallocator> p(
      (data_t *)aligned_malloc(size));
...
template <typename data_t>
inline void Copy(data_t *dst, const data_t *src, const size_t numItems) {
